...
def gen_rab_freq(n0=0, n1=1, omega0=1.0, eta=0.1):
    ...
    return (
        ...
        * eta ** (n_l - n_s)
        * eval_genlaguerre(n_s, n_l - n_s, eta * eta)
    )
...
def sim_rabi(t_vec, omega_vec, delta, n_bar):
    ...
    pop_vec = np.where(
        omega_eff != 0, (omega_vec / omega_eff * np.sin(omega_eff / 2 * t)) ** 2, 0
    )
...
def fitting_function(t, p):
    ...
    if not isinstance(p["omega_factor_vec"], np.ndarray):
        msg = (
            "param_dict['omega_factor_vec'] is expected to be a numpy "
...
if __name__ == "__main__":  # example & debug code
    ...
    # input parameters
    nmax, eta, delta_n, net_det, n_bar, omega_eff = 171, 1e-1, 0, 0.0, 10, 2.5e5
    ...
    y = sim_rabi(t, omega_eff_vec, net_det, n_bar) + np.random.random(t.shape) * 0.05
