...
def fitting_function(t, p_dict):
    ...
    """
    y = (
        np.exp(-p_dict["rate"] * (t - p_dict["t_dead"]))
        * (
            p_dict["a"]
            * np.sin((t - p_dict["t_dead"]) * p_dict["omega"] + p_dict["phi"])
            + p_dict["c_offset"]
        )
        + p_dict["c_equ"]
    )
...
def init_all(t, y, p_dict):
    ...
    if (
        "c_equ" in p_dict.constant_parameter_names
    ...
    p_dict["c_offset"] = np.mean(y[np.argwhere(t < t[0] + t_period)]) - p_dict["c_equ"]
...
def derived_params(p_dict, p_error_dict):
    ...
    p_dict["t_max_transfer"] = p_dict["t_dead"] + 1 / p_dict["omega"] * (
        np.pi / 2
        + (
            -p_dict["phi"]
    ...
    dtdrate = 1 / p_dict["omega"] * p_dict["rate"] / p_dict[
        "omega"
    ] ** 2 * temp0**3 * darcsindx(temp0) + temp4 * darcsindx(temp2)
    ...
    dtdomega = (
        1 / p_dict["omega"] * (-temp5 * darcsindx(temp0) + temp6 * darcsindx(temp2))
    )
