...
def optimise_readout(
    bright_rate, dark_rate, dark_to_bright_rate=(1 / 1.168), p_bright=0.5
):
...
def optimise_treshold(
    bright_rate, dark_rate, t_bin, dark_to_bright_rate=0.0, p_bright=0.5
):
    ...
    err_vec = np.array(
        [
            calc_p_error(
                bright_rate, dark_rate, t_bin, i, dark_to_bright_rate, p_bright=p_bright
            )
            for i in thresh_vec
        ]
    )
...
def optimise_t_bin(
    bright_rate, dark_rate, thresh_count, dark_to_bright_rate=0.0, p_bright=0.5
):
...
def calc_p_error(
    bright_rate, dark_rate, t_bin, thresh_count, dark_to_bright_rate=0.0, p_bright=0.5
):
    ...
    def p_n_given_dark(n):
        ...
        x_n *= gammainc(n + 1, eps * (rb_tau - 1)) - gammainc(
            n + 1, dark_rate * t_bin / rb_tau * (rb_tau - 1)
        )

        d_n = (
            poisson.pmf(n, mu=dark_rate * t_bin) * np.exp(-t_bin * dark_to_bright_rate)
            + x_n
        )
        return d_n
